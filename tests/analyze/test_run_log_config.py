...
VALID_CONFIG = {
    "type": "dict_config",
    "value": {
        "paths": {"root": "/scratch/ddr8143", "proj_dir_name": "cifar10_scratch"},
        "data": {
            "name": "cifar10",
            "num_workers": 8,
        },
...
def test_get_sweep_cfg_with_remap():
    ...
    assert (
        sweep_cfg.get("data.num_workers")
        == VALID_CONFIG["value"]["data"]["num_workers"]
    )
...
def test_error_invalid_type():
    ...
    bad_config = {"type": "str", "value": VALID_CONFIG["value"]}
    cfg_instance = RunLogConfig(bad_config)
    # Expect an error message about the type.
    assert (
        ">> Config json doesn't have {type: dict_config}" in cfg_instance.parse_errors
    )
