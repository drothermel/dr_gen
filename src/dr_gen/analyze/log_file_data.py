...
def get_train_time(train_time_json):
    if train_time_json.get("type", None) == "str" and "value" in timing_line:
...
class LogFileData:
    ...
    def parse(self):
        ...
        if contents is None:
            ...
            self.parse_errors.append(f">> File two lines or less, unable to parse")
            return
        ...
        self.metrics_by_split = get_logged_metrics_infer_epoch(
            self.config, contents
        )

        # Validate length of metrics
        expected_epochs = config.flat_cfg[config.epochs_key]
        all_xs = self.get_all_xs_flat()
        all_vals = self.get_all_vals_flat()
        if not all([len(xs) == expected_epochs for xs in all_xs.values()]):
            self.parse_errors.append(f">> Not all xs are the expected length")
        if not all([len(vals) == expected_epochs for vals in all_vals.values()]):
            self.parse_errors.append(f">> Not all vals are the expected length")
