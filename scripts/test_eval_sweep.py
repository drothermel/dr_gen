...
OUTPUT_CSV_PATH = (
    f"/Users/daniellerothermel/Desktop/comparison_sweep_summary_v0_n"
    f"{NUM_N_POINTS}_t{NUM_T_POINTS}.csv"
)
OUTPUT_PKL_PATH = (
    f"/Users/daniellerothermel/Desktop/comparison_sweep_summary_v0_n"
    f"{NUM_N_POINTS}_t{NUM_T_POINTS}.pkl"
)
...
def trim_hpm_data(hpm_data_dict, n_runs, t_length):
    """Trims the data within a dictionary.

    Takes {hpm_name: [[run1_metrics], [run2_metrics], ...]} to the specified number of
    runs and timesteps.
    ...
    Returns:
        dict: A new dictionary with the trimmed data. Returns empty dict if input
            is empty.
    """
    ...
    for hpm_name, runs_data in hpm_data_dict.items():
        ...
        if not isinstance(runs_data, list | np.ndarray) or not all(
            isinstance(run, list | np.ndarray) for run in runs_data
        ):
            print(
                f"Warning: Skipping HPM '{hpm_name}' due to unexpected data "
                f"format: {type(runs_data)}"
            )
            continue

        # Trim runs (outer list)
        actual_n = min(n_runs, len(runs_data))
        trimmed_runs = deepcopy(
            runs_data[:actual_n]
        ...
        for run in trimmed_runs:
            if not isinstance(
                run, list | np.ndarray
            ):  # Should not happen based on check above, but safety first
                continue
            actual_t = min(t_length, len(run))
            trimmed_hpm_data.append(run[:actual_t])

        # Only add if there's valid data after trimming
        if trimmed_hpm_data and all(
            trimmed_hpm_data
        ):  # Ensure no empty inner lists remained
...
(hpms_val_pre_full, hpms_eval_pre_full), (hpms_val_rand_full, hpms_eval_rand_full) = (
...
min_runs_pre = (
    min(len(runs) for runs in hpms_val_pre_full.values()) if hpms_val_pre_full else 0
)
min_runs_rand = (
    min(len(runs) for runs in hpms_val_rand_full.values()) if hpms_val_rand_full else 0
)
max_n = min(min_runs_pre, min_runs_rand)

# Find min timesteps across all runs for all HPMs (use validation set as proxy)
min_ts_pre = (
    min(len(ts) for runs in hpms_val_pre_full.values() for ts in runs)
    if hpms_val_pre_full
    else 0
)
min_ts_rand = (
    min(len(ts) for runs in hpms_val_rand_full.values() for ts in runs)
    if hpms_val_rand_full
    else 0
)
...
# Need at least 2 points to define a range
if max_n < MIN_DATA_POINTS_FOR_SWEEP or max_t < MIN_DATA_POINTS_FOR_SWEEP:
    print(
        f"Error: Insufficient data dimensions (max_n={max_n}, max_t={max_t}) "
        f"for sweep with >1 points. Exiting."
    )
    exit()
...
for n in n_values:
    ...
    for t in t_values:
        ...
        if (
            not hpms_val_pre
            or not hpms_eval_pre
            or not hpms_val_rand
            or not hpms_eval_rand
        ):
...
if all_results:
    print(
        f"Saving results summary for {len(all_results)} successful runs to "
        f"{OUTPUT_CSV_PATH}..."
    )
