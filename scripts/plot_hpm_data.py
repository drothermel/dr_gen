...
def load_data(pkl_filepath):
    ...
    try:
        ...
        print(f"Error: Could not unpickle data from {pkl_filepath}. File might be corrupted or not a pickle file.")
        return None
...
def get_user_choice(prompt, options, allow_skip=False, allow_done=False):
    ...
    print(prompt)
    # Display options. If an option is a list or dict, use json.dumps for cleaner display.
    display_options = []
    for opt in options:
...
def select_run_group_interactively(all_runs_data):
    ...
    for hpm_key in sorted_hpm_keys_to_consider:
        ...
        if len(candidate_run_names) == 1 and hpm_key not in user_selected_hpms:
            ...
            # Continue to the next HPM key; filtering will happen based on accumulated user_selected_hpms
        ...
        if len(actual_options_for_selection) == 1:
            ...
            if not candidate_run_names:
                 print("Error: Auto-selection or fixed HPM led to no candidates. This is unexpected.")
                 return None
            continue # Move to the next HPM key, as this one is now determined for the current scope

        # Prompt user for this HPM
        prompt_message = f"\nSelect value for '{hpm_key}' (currently {len(candidate_run_names)} matching groups):"
        user_choice_val = get_user_choice(prompt_message, actual_options_for_selection, allow_skip=True, allow_done=True)
    ...
    # Final filtering based on all user_selected_hpms (includes auto-selected and user-chosen)
    # This step is crucial because a user might "skip" an HPM, but subsequent choices for other HPMs
...
def plot_metrics(run_name, run_details, split_name, metric_name):
    ...
    if metric_data_SxE.size == 0: # Empty array
        print(f"Warning: Metric data for {metric_name} in {split_name} is empty. Cannot plot.")
        return


    num_seeds, num_epochs = metric_data_SxE.shape
    if num_epochs == 0:
        print(f"Warning: Metric data for {metric_name} in {split_name} has 0 epochs. Cannot plot.")
        return
...
def main():
    parser = argparse.ArgumentParser(description="Interactively plot metrics from aggregated log data.")
    parser.add_argument("--pkl_file", default=".", type=str, help="Path to the .pkl file containing aggregated run data.")
    args = parser.parse_args()
    args.pkl_file = "/Users/daniellerothermel/drotherm/data/dr_gen/run_data_v1/lr_wd_init_v0_t2.pkl"
